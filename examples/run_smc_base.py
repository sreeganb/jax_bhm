...
# -*- coding: utf-8 -*-
"""
Modular **base** SMC simulation with selectable mutation kernel (RMH or HMC).

This script mirrors ``run_smc_modular.py`` but uses ``blackjax.smc.base``
instead of ``blackjax.adaptive_tempered_smc``.  The key differences are:

    1. A **fixed** temperature schedule (linear / geometric / sigmoid) is
       used instead of an ESS-adaptive one.
    2. Step sizes for the MCMC kernel are **not** adapted between
       temperature steps.
    3. The user controls the granularity of tempering via
       ``--n_temperature_steps`` and ``--schedule``.

Scoring components (identical to run_smc_modular.py):
    - scoring/compute_ccc.py   : Density map generation and CCC calculation
    - scoring/em_log_likelihood.py : Log-likelihood from CCC
...
Probabilistic model:
    ...
    likelihood = p(CCC | sigma) * excluded_volume

    SMC tempers:  pi_t(x)  ∝  prior(x) * likelihood(x)^{lambda_t}
    with lambda on a fixed schedule from 0 → 1.

Usage:
    python run_smc_base.py --kernel rmh --n_particles 100 --schedule linear
    python run_smc_base.py --kernel hmc --n_particles 50  --schedule geometric
"""
...
from sampling.smc_base_sampler import (
    run_base_smc_rmh,
    run_base_smc_hmc,
    get_smc_samples,
    get_best_sample,
...
@dataclass
class BaseSMCConfig:
    """Configuration for base (non-adaptive) SMC simulation."""
    # Kernel selection
    ...
    # Grid setup (computed from map header)
    ...
    # === Base SMC-specific parameters ===
    n_temperature_steps: int = 20   # Fixed number of tempering steps
    schedule: Literal['linear', 'geometric', 'sigmoid'] = 'linear'
    ...
    # Output
    output_dir: str = "output_smc_base"
    output_prefix: str = "smc_base"
...
def setup_logging(config: BaseSMCConfig) -> logging.Logger:
    ...
    # Create logger
    logger = logging.getLogger("smc_base")
...
def run_smc_simulation(config: BaseSMCConfig) -> Dict[str, Any]:
    """
    Run base SMC simulation with the specified configuration.
    ...
    logger.info("=" * 60)
    logger.info(f"Base SMC Simulation with {config.kernel.upper()} Mutation Kernel")
    logger.info(f"Schedule: {config.schedule} ({config.n_temperature_steps} steps)")
    ...
    logger.info(f"  Shape: {map_shape}, Voxel: {voxel_size:.2f} A")
    logger.info(f"  Origin: [{origin[0]:.1f}, {origin[1]:.1f}, {origin[2]:.1f}]")

    # Reconstruct bins from MRC header
    binsx = jnp.linspace(
    ...
    logger.info(f"  Distance (density COM <-> ideal COM): {com_distance:.1f} A")
    ...
    if com_distance > 50.0:
        logger.warning(f"  Density COM is {com_distance:.0f} A from ideal coords COM.")
        logger.warning(f"  This may indicate an alignment issue.")
    else:
        logger.info(f"  OK Density COM and ideal coords COM are within {com_distance:.1f} A")
    ...
    # Initialize particles near the DENSITY COM
    ...
    logger.info(f"Particles initialized near density COM (spread ~ {spread} A)")
    ...
    # 7. Run base SMC
    # =========================================================================
    timer.start("7. Base SMC sampling")

    logger.info(f"Base SMC configuration:")
    logger.info(f"  Kernel:             {config.kernel.upper()}")
    logger.info(f"  MCMC steps/temp:    {config.n_mcmc_steps}")
    logger.info(f"  Temperature steps:  {config.n_temperature_steps}")
    ...
    if config.kernel == 'rmh':
        ...
        final_state, info_history, best_positions, best_scores, lambdas = run_base_smc_rmh(
            log_prior_fn=log_prior_fn,
            log_likelihood_fn=log_likelihood_fn,
            log_prob_fn=log_prob_fn,
            initial_positions=initial_positions,
            rng_key=smc_key,
            n_temperature_steps=config.n_temperature_steps,
        ...
        final_state, info_history, best_positions, best_scores, lambdas = run_base_smc_hmc(
            log_prior_fn=log_prior_fn,
            log_likelihood_fn=log_likelihood_fn,
            log_prob_fn=log_prob_fn,
            initial_positions=initial_positions,
    ...
    timer.stop("7. Base SMC sampling")
    ...
    if best_positions is not None and best_scores is not None:
        ...
        logger.info("=" * 60)
        logger.info(f"{'Step':<8} {'Lambda':>8} {'Score':>12} {'CCC':>12} {'Coord Disp':>14}")
        logger.info("-" * 60)
        ...
        for step_idx, (score, ccc) in enumerate(zip(best_scores, best_cccs)):
            ...
            lam_val = float(lambdas[step_idx]) if step_idx < len(lambdas) else 1.0
            logger.info(
                f"{step_idx:<8} {lam_val:>8.4f} {float(score):>12.2f} "
                f"{float(ccc):>12.4f} {disp:>14.2f}"
            )
        ...
        logger.info(f"Total displacement (first->last best): {final_disp:.2f} A")
        ...
        logger.info(f"Per-particle displacement: min={per_particle_disp.min():.2f}, "
                    f"max={per_particle_disp.max():.2f}, mean={per_particle_disp.mean():.2f} A")
    ...
    if config.save_trajectory and best_positions is not None and best_scores is not None:
        ...
        save_mcmc_to_hdf5(
            ...
            system,
            params={
                'method': f'BlackJAX_BaseSMC_{config.kernel.upper()}',
                ...
                'temperature_schedule': lambdas.tolist(),
            },
    ...
    summary = {
        ...
        'results': {
            ...
            'n_smc_steps': config.n_temperature_steps,
            'schedule': config.schedule,
            'temperature_schedule': lambdas.tolist(),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Base SMC simulation (fixed schedule, no adaptive step sizes) "
                    "with selectable mutation kernel (RMH or HMC)."
    )
    ...
    parser.add_argument('--hmc_step_size', type=float, default=0.01,
                        help='HMC leapfrog step size (fixed)')
    parser.add_argument('--hmc_L', type=int, default=10,
                        help='HMC number of leapfrog steps')

    # Output
    parser.add_argument('--output_dir', type=str, default='output_smc_base',
                        help='Output directory')
    parser.add_argument('--output_prefix', type=str, default='smc_base',
...
def main():
    ...
    print(f"Schedule:    {config.schedule} ({config.n_temperature_steps} steps)")
    print("=" * 60)
